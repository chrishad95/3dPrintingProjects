...
The standard design featuring a low-profile flat top lip that rests flush on the pipe rim.
- **Flange/Lip Outer Diameter**: $50.8\text{ mm}$ (matches pipe OD)
- **Flange/Lip Height**: $5.0\text{ mm}$ (portion of cap staying outside the pipe)
- **Insert Depth (Plug Height)**: $20.0\text{ mm}$ (portion inside the pipe)
...
- **Flange Height**: $8.0\text{ mm}$ (total height staying outside the pipe)
  - **Vertical Edge/Rim**: $2.0\text{ mm}$ (vertical edge to ensure a robust, non-fragile perimeter)
  - **Spherical Dome**: $6.0\text{ mm}$ (portion curving upward above the vertical rim)
- **Insert Depth (Plug Height)**: $17.0\text{ mm}$ (portion inside the pipe)
- **Dome Radius (Outer)**: $56.7633\text{ mm}$ (spherical cap formula: $R = \frac{r^2 + h^2}{2h}$)
- **Internal Cavity Style**: Double-curved cavity with a matching $2.0\text{ mm}$ cylindrical rim cavity and an internal dome ($2.5\text{ mm}$ height, $76.4735\text{ mm}$ radius) to maintain a perfectly uniform $3.5\text{ mm}$ wall thickness.
- **Slicing/Print Orientation**:
